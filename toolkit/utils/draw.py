...
class ChartBase:
    ...
    def save(self, fig_name: str = "default_name", form: str = "eps", save_dir: Path | str = Path("figures")):
        if isinstance(save_dir, str):
            save_dir = Path(save_dir)
        if not save_dir.exists():
            save_dir.mkdir(parents=True)
        full_name = fig_name + "." + form
        self.fig.savefig(save_dir / full_name, format=form, bbox_inches="tight", dpi=self.dpi)
