...
def rand_permutation(data_A, data_B, n, R):
    ...
    for x in range(0, R):
        temp_A = data_A
        temp_B = data_B
        samples = [np.random.randint(1, 3) for _ in range(n)]  # which samples to swap without repetitions
...
if __name__ == "__main__":
    baseline_data = [91.185, 91.018, 91.548, 90.832, 91.168, 91.882]
    mod1_data = [91.462, 91.486, 91.660, 91.195, 91.492, 91.110]  # 待测试
    mod2_data = [91.794, 91.785, 91.634, 91.499, 91.603, 91.966]
    print("f1: ")
    print(f"base: {sum(baseline_data)/len(baseline_data)}\nmod1: {sum(mod1_data)/len(mod1_data)}\nmod2: {sum(mod2_data)/len(mod2_data)}\n")
    main(baseline_data, mod2_data, 0.05, "t-test")
