...
if [ -z "$pct_used" ] || [ "$pct_used" = "null" ]; then
    pct_used=0
fi
...
line1+="ctx ${ctx_bar} ${pct_color}${pct_used}%${reset}"
line1+="${sep}"
line1+="${skip_perms}${cyan}${dirname}${reset}"
if [ -n "$git_branch" ]; then
    line1+=" ${green}(${git_branch}${red}${git_dirty}${green})${reset}"
fi
if [ -n "$api_duration_ms" ] && [ "$api_duration_ms" != "0" ]; then
    api_duration_fmt=$(format_ms "$api_duration_ms")
    if [ -n "$api_duration_fmt" ]; then
        line1+="${sep}"
        line1+="${dim}⏱ ${reset}${white}${api_duration_fmt}${reset}"
    fi
fi
line1+="${sep}"
case "$effort" in
